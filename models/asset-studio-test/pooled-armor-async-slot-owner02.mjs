...
async function sourceDescriptor(a, index = false) {
  const original = {
    itemSize: a.itemSize,
    count: a.count,
  ...
  if (a.array.constructor === Uint8Array || a.array.constructor === Uint16Array) {
    const widened = new Uint32Array(a.array);
    let restartRewrites = 0;
    if (index) {
      for (let i = 0;i < widened.length; i++)
        if (widened[i] === 65535) {
          widened[i] = 4294967295;
          restartRewrites++;
        }
    }
    original.webgpuUint32 = {
      type: "Uint32Array",
      sha256: await digest(arrayBytes(widened)),
      rule: index ? "three0.183.2-uint-widen-index-ffff-restart" : "three0.183.2-uint-widen-vertex",
      restartRewrites
    };
  }
  return original;
  ...
  for (const node of doc.nodes.filter((n) => n.mesh !== undefined)) {
    ...
    for (const p of doc.meshes[node.mesh].primitives) {
      ...
      for (const [semantic, index2] of Object.entries(p.attributes)) {
        ...
        attributes[ATTRIBUTE_NAMES[semantic] ?? semantic.toLowerCase()] = await sourceDescriptor(a);
      }
      const index = attribute(p.indices);
      primitives.push({
        attributes,
        index: await sourceDescriptor(index, true)
      });
    }
...
async function installTorsoCpu8({ THREE, root, skeleton, source }) {
  ...
  async function checkAttribute(actual, expected, label) {
    const array = actual?.array, actualType = array?.constructor.name;
    const context = JSON.stringify({
      ...
      sourceCanonicalSHA256: expected.sha256
    });
    require2(actual && !actual.isInterleavedBufferAttribute && !actual.normalized && actual.itemSize === expected.itemSize && actual.count === expected.count && array.length === expected.itemSize * expected.count && (actualType === expected.type || actualType === expected.webgpuUint32?.type), "Source attribute schema changed: " + context);
    const actualSHA256 = await digest(Uint8Array.from(arrayBytes(array)));
    const widened = actualType !== expected.type;
    const expectedSHA256 = widened ? expected.webgpuUint32.sha256 : expected.sha256;
    require2(actualSHA256 === expectedSHA256, "Source attribute bytes changed: " + JSON.stringify({ ...JSON.parse(context), actualSHA256, expectedSHA256 }));
    checkedAttributes++;
    if (widened)
      promoted.push(Object.freeze({
        ...
        sourceType: expected.type,
        sourceCanonicalSHA256: expected.sha256,
        backendCanonicalSHA256: expected.webgpuUint32.sha256,
        rule: expected.webgpuUint32.rule,
        restartRewrites: expected.webgpuUint32.restartRewrites
      }));
  }
  for (const expected of record.owners) {
    ...
    for (let i = 0;i < actual.length; i++) {
      ...
      for (const [name, description] of Object.entries(p.attributes))
        await checkAttribute(mesh.geometry.attributes[name], description, expected.name + "/primitive" + i + "/" + name);
      await checkAttribute(mesh.geometry.index, p.index, expected.name + "/primitive" + i + "/indices");
    }
  }
  require2(seen.size === meshes.length, "Unaccounted or dummy meshes");
  const adapter = installCore({
    THREE,
    root,
    skeleton,
  ...
  Object.defineProperty(adapter.metrics, "sourceAttestation", {
    enumerable: true,
    value: Object.freeze({
      profile: record.profile,
      ...
      scope: "Exact observed original or canonical Three0.183.2 widened bytes; source GLB unchanged. Not a claim of byte-identical runtime storage or an observed GPU upload."
    })
  });
  return adapter;
}
